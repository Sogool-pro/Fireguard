...
#include <TimeLib.h>
#include <Wire.h>
#include <LiquidCrystal_I2C.h> // Using ESP32-compatible version
...
#define DISPLAY_CYCLE_INTERVAL 5000 // Cycle displays every 5 seconds
...
const String PHONE_NUMBERS[] = {"+639306702771", "+639338765297"};
...
void setup() {
  ...
  // Initialize SIM800L with improved initialization
  ...
  sendATCommand("AT+CSQ", "OK", 2000); // Check signal quality
  sendATCommand("AT+CREG?", "OK", 2000); // Check network registration

  // Configure static IP
  IPAddress local_IP(192, 168, 3, 108);
  ...
  // Verify the IP address is what we set
  if (WiFi.localIP() != local_IP) {
    Serial.println("\nWarning: Failed to get static IP, using DHCP-assigned IP instead");
    lcd.setCursor(0, 2);
    lcd.print("DHCP IP: ");
  } else {
    lcd.setCursor(0, 2);
...
void updateLCD() {
  ...
  // Check if it's time to cycle display modes
  if (now - lastDisplayChange >= DISPLAY_CYCLE_INTERVAL) {
    cycleDisplayMode();
...
void checkIncomingSMS() {
  if (sim800l.available()) {
    ...
    if (response.indexOf("+CMT:") != -1) {
      ...
      // You can add code here to process incoming SMS if needed
    }
  }
}
...
void evaluateAlertLevel(NodeData* nodeData, NodeAlert* nodeAlert) {
  ...
  if (newLevel != nodeData->alertLevel) {
    ...
    // For any alert level above NORMAL, send notification
    if (newLevel != NORMAL && shouldSendAlert(nodeData, nodeAlert)) {
      ...
      if (now - nodeAlert->warningStartTime >= WARNING_DELAY) {
        ...
        if (tempWarning || gasWarning || coWarning || humidityWarning) {
          ...
          if (shouldSendAlert(nodeData, nodeAlert)) {
            sendAlertToFirebase(nodeData, nodeAlert);
            // Only send SMS if we haven't sent one recently for this alert
            if (now - nodeAlert->lastSMSTime > SMS_COOLDOWN) {
              sendSMSAlert(nodeData, nodeAlert);
              nodeAlert->smsSent = true;
              nodeAlert->lastSMSTime = now;
            }
...
void sendSMSAlert(NodeData* nodeData, NodeAlert* nodeAlert) {
  // Only send SMS if buzzer is not silenced
  if (!nodeAlert->buzzerSilenced) {
    ...
    String msg = emoji + " " + prefix + ": " + nodeData->alertMessage + " " + emoji + "\nNode: " + nodeData->nodeID +
...
void loop() {
  ...
  checkButtons();
  checkIncomingSMS(); // Check for incoming SMS messages
